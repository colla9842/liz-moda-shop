    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await api.post('/api/cuentas', {
                ...
                moneda: '66a93eb72c6ec1a55e88aadb', // Assuming this is a valid moneda ID
    ...
    return (
        <div className="min-h-screen p-4 sm:p-8">
            <Card className="max-w-4xl mx-auto bg-white/50 backdrop-blur-md shadow-lg">
                <CardHeader>
                    <CardTitle className="text-xl sm:text-2xl font-bold text-gray-800">Agregar Venta</CardTitle>
                </CardHeader>
                <CardContent>
                    {alertMessage && (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <Dialog open={modalShow} onOpenChange={setModalShow}>
                                ...
                                <DialogContent className="w-[90vw] max-w-[700px] h-[90vh] max-h-[600px] overflow-hidden flex flex-col bg-white/90 backdrop-blur-md">
                                    <DialogHeader>
                                                ...
                                                return (
                                                    <Card key={product._id} className={`overflow-hidden ${isSelected ? 'opacity-50' : ''}`}>
                                                        ...
                                                        <CardContent className="p-2 sm:p-4">
                                                            <h4 className="font-semibold mb-1 sm:mb-2 text-xs sm:text-sm">{product.nombre}</h4>
                                                            <p className="text-xs sm:text-sm text-gray-600 mb-1 sm:mb-2">${product.precio_venta_usd}</p>
                                                            <Button
                            ...
                            {saleData.productos.map((prod, index) => {
                                const producto = productos.find(p => p._id === prod.articulo);
                                return (
                                    <div key={index} className="flex items-center space-x-4 mt-2">
                                        <Input
                                            type="text"
                                            readOnly
                                            value={producto?.nombre || 'Seleccionar producto'}
                                            className="flex-grow"
                                        />
                                        <Input
                                            type="number"
                        ...
                        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                            ...
                            <Select
                                name="moneda"
                                onValueChange={(value) => handleChange({ target: { name: 'moneda', value } })}
                            >
                        ...
                        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                            ...
                            {saleData.cambio > 0 && (
                                <Select
                                    name="monedaCambio"
                                    onValueChange={(value) => handleChange({ target: { name: 'monedaCambio', value } })}
                                >
                                    <SelectTrigger className="w-full sm:w-[180px]">
                        ...
                        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                            <Select
                                name="gestor"
                                onValueChange={(value) => handleChange({ target: { name: 'gestor', value } })}
                            >
