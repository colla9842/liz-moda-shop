...
export function AddSaleComponent() {
    ...
    const handleSelectProduct = (product) => {
        setSaleData(prevState => (Object.assign(Object.assign({}, prevState), { productos: [...prevState.productos, { articulo: product._id, cantidad: 1 }] })));
    };
    const handleProductChange = (index, field, value) => {
        const updatedProducts = [...saleData.productos];
        updatedProducts[index] = Object.assign(Object.assign({}, updatedProducts[index]), { [field]: value });
        setSaleData(prevState => (Object.assign(Object.assign({}, prevState), { productos: updatedProducts })));
    };
    const handleRemoveProduct = index => {
        setSaleData(prevState => (Object.assign(Object.assign({}, prevState), { productos: prevState.productos.filter((_, i) => i !== index) })));
    };
    const handleChange = e => {
        const { name, value } = e.target;
        setSaleData(prevState => (Object.assign(Object.assign({}, prevState), { [name]: value })));
    };
    ...
    const filteredProducts = productos.filter(product => product.nombre.toLowerCase().includes(searchQuery.toLowerCase()) &&
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex items-center space-x-4">
              ...
              <Input type="date" id="fecha" name="fecha" value={saleData.fecha} onChange={handleChange} required className="flex-grow"/>
            </div>
              ...
              <Dialog open={modalShow} onOpenChange={setModalShow}>
                ...
                <DialogContent className="sm:max-w-[700px] sm:h-[80vh] overflow-hidden flex flex-col bg-white/90 backdrop-blur-md">
                  ...
                  <div className="flex items-center space-x-2 p-4">
                    ...
                    <Input type="text" placeholder="Buscar productos..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-grow"/>
                  </div>
                  <ScrollArea className="flex-grow">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-4">
                      {filteredProducts.map(product => {
            const isSelected = saleData.productos.some(p => p.articulo === product._id);
            return (<Card key={product._id} className={`overflow-hidden ${isSelected ? 'opacity-50' : ''}`}>
                            <div className="relative h-40 w-full">
                              <Image src={product.imagen || '/placeholder.svg'} alt={product.nombre} layout="fill" objectFit="cover"/>
                            </div>
                            <CardContent className="p-4">
                              ...
                              <Button onClick={() => handleSelectProduct(product)} size="sm" className="w-full" disabled={isSelected}>
            ...
            var _a;
            return (<div key={index} className="flex items-center space-x-4 mt-2">
                  <Input type="text" readOnly value={((_a = productos.find(p => p._id === prod.articulo)) === null || _a === void 0 ? void 0 : _a.nombre) || 'Seleccionar producto'} className="flex-grow"/>
                  <Input type="number" value={prod.cantidad} onChange={(e) => handleProductChange(index, 'cantidad', e.target.value)} min="1" required className="w-24"/>
            ...
            <div className="flex space-x-4">
              <div className="flex items-center space-x-2 flex-grow">
                ...
                <Input type="number" id="precioTotal" name="precioTotal" value={saleData.precioTotal} onChange={handleChange} required placeholder="Precio Total"/>
              </div>
              <Select name="moneda" onValueChange={(value) => handleChange({ target: { name: 'moneda', value } })}>
                ...
                <SelectContent>
                  {monedas.map(moneda => (<SelectItem key={moneda._id} value={moneda._id}>{moneda.nombre}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>

            <div className="flex space-x-4">
              <Input type="number" id="cambio" name="cambio" value={saleData.cambio} onChange={handleChange} placeholder="Cambio" className="flex-grow"/>
              {saleData.cambio > 0 && (<Select name="monedaCambio" onValueChange={(value) => handleChange({ target: { name: 'monedaCambio', value } })}>
                  <SelectTrigger className="w-[180px]">
            ...
            <div className="flex space-x-4">
              <Select name="gestor" onValueChange={(value) => handleChange({ target: { name: 'gestor', value } })}>
                ...
                <SelectContent>
                  {gestores.map(gestor => (<SelectItem key={gestor._id} value={gestor._id}>{gestor.nombre}</SelectItem>))}
                  <SelectItem value="nuevo">Crear nuevo gestor</SelectItem>
                </SelectContent>
              </Select>
              <Input type="number" id="comision" name="comision" value={saleData.comision} onChange={handleChange} required placeholder="Comisión" className="w-32"/>
            </div>

            {saleData.gestor === 'nuevo' && (<div className="flex items-center space-x-2">
                <Users className="text-gray-500"/>
                <Input type="text" id="nuevoGestor" name="nuevoGestor" value={saleData.nuevoGestor} onChange={handleChange} placeholder="Nombre del nuevo gestor" className="flex-grow"/>
              </div>)}
