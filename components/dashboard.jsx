...
export function DashboardComponent() {
    ...
    const calculateTotalUSD = (balances, currencies, setTotal) => {
        var _a;
        const cupToUsdRate = ((_a = currencies.find(c => c.nombre === 'USD')) === null || _a === void 0 ? void 0 : _a.tasaCambio) || 1;
    ...
    const sortData = (data) => {
        return Object.entries(data).sort((a, b) => {
            ...
            return 0;
        }).filter(([currency]) => currency.toLowerCase().includes(filter.toLowerCase()));
    };
    ...
    const renderTable = (data, title) => (<Card className="bg-white/50 backdrop-blur-md border-none shadow-lg">
      <CardHeader>
        <CardTitle className="text-xl font-bold text-gray-800">{title}</CardTitle>
        <Input placeholder="Filtrar por moneda" value={filter} onChange={(e) => setFilter(e.target.value)} className="max-w-sm bg-white/70"/>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {sortData(data).map(([currency, amount]) => (<TableRow key={currency} className="hover:bg-white/40 transition-colors">
            ...
            {sortData(data).length === 0 && (<TableRow>
