...
export function ExpenseByCategoryComponent() {
    ...
    const handleCategoryChange = (expenseId, category) => {
        setCategoryMap(prev => (Object.assign(Object.assign({}, prev), { [expenseId]: category })));
    };
    ...
    const filteredExpenses = expenses
        ...
        .filter(expense => selectedCategory === 'Sin Categorizar'
        ? !expense.categoria || expense.categoria === 'Sin Categorizar'
        : expense.categoria === selectedCategory);
    const renderExpenseTable = () => (<div className="overflow-x-auto">
      <Table>
        <TableHeader>
        ...
        <TableBody>
          {filteredExpenses.map(expense => (<TableRow key={expense._id} className="border-b border-white/10">
              ...
              <TableCell className="text-right">
                <div className="flex items-center justify-end space-x-2">
                  <Select value={categoryMap[expense._id] || expense.categoria || 'Sin Categorizar'} onValueChange={(value) => handleCategoryChange(expense._id, value)}>
                    ...
                    <SelectContent className="bg-[#FF885B]/80 text-white border-white/20">
                      {categories.map(cat => (<SelectItem key={cat} value={cat} className="hover:bg-white/20">{cat}</SelectItem>))}
                    </SelectContent>
                  </Select>
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 mb-4">
            <div className="flex items-center space-x-2 w-full md:w-1/2">
              ...
              <Input type="text" placeholder="Buscar por descripción..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 bg-white/10 text-white placeholder-white/50 border-white/20"/>
            </div>
            <Select value={selectedCategory} onValueChange={setSelectedCategory}>
              ...
              <SelectContent className="bg-[#FF885B]/80 text-white border-white/20">
                {categories.map(category => (<SelectItem key={category} value={category} className="hover:bg-white/20">{category}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          {renderExpenseTable()}
