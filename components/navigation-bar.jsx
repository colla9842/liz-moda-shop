...
export function NavigationBarComponent() {
    ...
    const NavItem = useCallback(({ href, children, icon: Icon }) => (<Link href={href} className="flex items-center space-x-2 text-gray-200 hover:text-white transition-colors duration-200">
      <Icon className="w-5 h-5"/>
      <span>{children}</span>
    </Link>), []);
    const NavDropdown = useCallback(({ title, items, icon: Icon }) => (<DropdownMenu>
      ...
      <DropdownMenuContent className="bg-[#FF885B]/60 border-gray-700" sideOffset={5}>
        {items.map((item, index) => (<DropdownMenuItem key={index} className="focus:bg-gray-700/50" onSelect={() => router.push(item.href)}>
    ...
    return (<nav className="bg-[#FF885B]/60 backdrop-blur-md shadow-md z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden sm:ml-6 sm:flex sm:items-center space-x-4">
            ...
            <NavDropdown title="Reportes" icon={FileText} items={[
            ...
            <NavDropdown title="Cuentas" icon={DollarSign} items={[
            ...
            <NavDropdown title="Productos" icon={Package} items={[
          ...
          <div className="hidden sm:ml-6 sm:flex sm:items-center">
            {isLoggedIn ? (<DropdownMenu>
              ...
              </DropdownMenu>) : (<Link href="/login" className="text-gray-200 hover:text-white hover:bg-white/10 px-3 py-2 rounded-md text-sm font-medium">
                Iniciar Sesión
              </Link>)}
          </div>
